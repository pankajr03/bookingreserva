...
function specialDayTpl(?StaffSpecialDayResponse $day = null)
{
    $date = $day !== null ? $day->getDateFormatted() : '';
    $id = $day !== null ? $day->getId() : 0;
    $timesheet = $day !== null ? $day->getTimesheet() : [];
    ...
    if (empty($timesheet)) {
        ...
        $startTime = $timesheet['start'] ?? '';
        $endTime = $timesheet['end'] ?? '';
...
<div class="special-days-area">
    <?php
    foreach ($parameters->getSpecialDays() as $specialDay) {
        specialDayTpl($specialDay);
    }
?>
</div>
